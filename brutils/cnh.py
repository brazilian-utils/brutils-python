def is_valid_cnh(cnh: str | None) -> bool:
    ...
    cnh_digits, verification_cnh_digits = _get_cnh_number_and_last_two_digits(
        cnh
    )
    ...
    second_validation_digit = _get_eleventh_digit(
        digits, cnh_digits, first_validation_digit
    )

    return _validate_cnh(
        verification_cnh_digits, first_validation_digit, second_validation_digit
    )
...
def _get_eleventh_digit(digits, cnh_digits, first_validation_digit):
    ...
    if first_validation_digit > 9:
        second_validation_digit = (
            second_validation_digit + 9
            if (second_validation_digit - 2) < 0
            else second_validation_digit - 2
        )
...
def _validate_cnh(
    ...
    calculated_cnh_digits = [
        str(first_validation_digit),
        str(second_validation_digit),
    ]
