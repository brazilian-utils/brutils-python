...
def is_valid_ghana_tin(tin):
    ...
    if not any(
        tin.startswith(prefix) for prefix in ["P00", "C00", "G00", "Q00", "V00"]
    ):
...
def calculate_check_digit(base_number):
    ...
    total_sum = sum(
        int(d) * w for d, w in zip(base_number, weights[: len(base_number)])
    )
    ...
    return (
        str(check_digit)
        if check_digit != 10
        else choice("ABCDEFGHJKLMNPQRSTUVWXYZ")
    )
...
def example_usage():
    ...
    print(
        f"Generated GhanaCard PIN: {ghanacard} - Valid? {is_valid_ghanacard_pin(ghanacard)}"
    )
    ...
    if any(
        ...
        for prefix in ["P00", "C00", "G00", "Q00", "V00"]
    ):
        ...
        print(
            "Invalid tin format: Must be either a valid GRA TIN or GhanaCard PIN."
        )
