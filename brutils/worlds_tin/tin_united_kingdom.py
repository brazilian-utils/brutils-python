...
def format_nino(nino):  # type: (str) -> str
    ...
    if (
        len(nino) == 9
        and nino[:2].isalpha()
        and nino[2:8].isdigit()
        and nino[8].isalpha()
    ):
        return "{} {} {} {} {}".format(
            nino[:2], nino[2:4], nino[4:6], nino[6:8], nino[8]
        )
    return None
...
def generate_companies_house_registration_number(company_type="UK") -> str:
    ...
    if company_type == "UK":  # For English/Welsh companies (8 digits)
        ...
        company_type == "LLP"
    ):  # For Limited Liability Partnerships (OC/SL/NL + 6 digits)
...
def is_valid_nino(nino):  # type: (str) -> str
    ...
    if not (nino[:2].isalpha() and nino[2:8].isdigit() and nino[8].isalpha()):
        return (
            "Invalid NINO: Format should be 2 letters, 6 digits, and 1 letter."
        )
...
def is_valid_companies_house_registration_number(reg_number):  # type: (str) -> str
    ...
    if len(reg_number) == 8 and reg_number.isdigit():
        ...
        len(reg_number) == 8
        and reg_number[:2] in {"SC", "NI"}
        and reg_number[2:].isdigit()
    ):
        return "Valid Scottish or Northern Irish company registration number."
    elif (
        len(reg_number) == 8
        and reg_number[:2] in {"OC", "SL", "NL"}
        and reg_number[2:].isdigit()
    ):
...
def generate_and_display():
    ...
    if type_choice == "NIN":
        ...
        return (
            format_nino(generated)
            if is_valid_nino(generated) == "Valid NINO"
            else "Invalid NIN"
        )
        ...
        return (
            generated
            if is_valid_utr(generated) == "Valid UTR"
            else "Invalid UTR"
        )
        ...
        print(
            f"Generated Company Registration Number: {generated} - Valid? {is_valid_companies_house_registration_number(generated)}"
        )
        return generated
...
user_input = input(
    "Enter a number (NINO, UTR, or Company Registration Number): "
)
